...
try {
  ...
  // First load adventure history (this should always succeed)
  await this.loadAdventureHistory();
  
  // Then try to load active adventure (this might fail)
  try {
    ...
    console.error('Error loading active adventure:', error);
    // This is okay - the character might not have an active adventure
    this._showStartAdventure();
  }
} catch (error) {
  console.error('Error in loadAdventureData:', error);
...
try {
  console.log(`Loading adventure history for character: ${window.GameState.selectedCharacter.id}`);
  const adventures = await window.API.getAdventureHistory(window.GameState.selectedCharacter.id);
  console.log(`Received ${adventures.length} adventures:`, adventures);
  
  // Filter out active adventures since we handle those separately
  const completedAdventures = adventures.filter(adv => adv.status !== 'active');
  
  // Update game state with history
  window.GameState.setAdventures(completedAdventures);
  
  // Update UI
  if (window.AdventureUI && typeof window.AdventureUI.renderAdventureHistory === 'function') {
    window.AdventureUI.renderAdventureHistory(completedAdventures);
  }
} catch (error) {
